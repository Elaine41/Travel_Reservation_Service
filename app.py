...
# please input the database credentials here
...
def sc_flight():
         ...
         #cursor.close()
         query2="Select airport_id from airport"
         cursor.execute(query2)
         airports_=cursor.fetchall()
         airports_=list(airports_)
         airports_=[a[0] for a in airports_]
         #cursor.execute(query2)
         
         query3="select flight_num,airline_name from flight"
         ...
         if request.method=="POST":
           ...
           f_airport=request.form['from_airports_']
           t_airport=request.form['to_airports_']
           ...
           if request.form.get('sc')=='Schedule Flight':
                ...
                if t_airport !=f_airport and (fno,airline_) not in tb:
                 ...
                 print(res)
                    ...
                    return render_template("sc_flight.html",airlines=airlines,cur_date=cur_date,airports_=airports_)
...
def view_airport():
    cursor = db_connection.cursor()
    q=("SELECT Airport_Id , Airport_Name , Time_Zone , concat(Street,' ', City ,' ', State ,' ', Zip) as Address from airport")
    cursor.execute(q)
    ...
    if request.method=="POST":
        ...
        if id:
           q=("SELECT Airport_Id , Airport_Name , Time_Zone , concat(Street,' ', City,' ', State,' ', Zip) as Address from airport WHERE Airport_Id=%s")
           cursor.execute(q,(id,))
           df=cursor.fetchall()
           df=list(df)
        ...
        if tz:
           q=("SELECT Airport_Id , Airport_Name , Time_Zone , concat(Street,' ', City,' ', State,' ', Zip) as Address from airport WHERE Time_Zone=%s")
           cursor.execute(q,(tz,))
           dh=cursor.fetchall()
           dh=list(dh)
        ...
        if request.form.get("_back")=="Back":
              return redirect("/admin")
...
@app.route('/owner/view_property_reservations', methods=["GET", "POST"])
def view_property_reservations():
    ...
    owner_email = session["username"]
        # owner_email = 'cbing10@gmail.com'
    q = """select reserve.Property_Name, Start_Date, End_Date, reserve.Customer, Phone_Number, case
            when was_cancelled = 1 then ((end_date-start_date) + 1) * Cost * 0.2 else ((end_date-start_date) + 1) * Cost end as total_booking_cost,
            score, content as review from reserve left join clients on reserve.customer=clients.email
            ...
            where reserve.owner_email = %s """
    
    cursor.execute(q, (owner_email,))
    a_data = cursor.fetchall()
...
def cancel_property():
    ...
    query1 = "select start_date, p.property_name, p.owner_email, concat(street,' ',city,' ',state, ' ',zip) as address from reserve r join property p on (p.property_name, p.owner_email)=(r.property_name, r.owner_email) where was_cancelled =0 and start_date>curdate() and customer=%s;"
...
def owner_deletes_account():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('da')=="Delete account":
            q="SELECT EXISTS(SELECT * FROM  property where Owner_email= %s)"
            cursor.execute(q,(owner_email,))
            property=cursor.fetchall()
            property= property[0][0]
            if property==1:
                msg="Delete failed. You still own properties."
                print(msg)
            else:
...
def view_flights():
        ...
        customer = session['username']
        q = ("select flight_id, airline, destination, flight_date, Num_Seats, seat_cost, total_spent, num_empty_seats as capacity from view_flight, Book where (flight_id, airline) = (Flight_Num, airline_name) and Customer = %s;")
        ...
        if request.method == "POST":
            ...
            if 'submit' in request.form:
                query = ("select flight_id, airline, destination, flight_date, Num_Seats, seat_cost, total_spent, num_empty_seats as capacity from view_flight, Book where (flight_id, airline) = (Flight_Num, airline_name) and flight_id = %s and customer = %s;")
        ...
        if 'submit_button' in request.form:
            ...
            if num_seats <= capacity:
                q3 = 'select * FROM view_flight, book where (flight_num, airline_name) = (flight_id, airline) and ' \
                     'flight_date = %s and customer = %s and was_cancelled = 0 and (flight_num, airline_name) <> (%s, %s)'
                cursor.execute(q3,(flight_date, customer,flight_num,airline_name))
...
def customer_rate_owner():
    ...
    query2='select distinct owner_email from reserve where was_cancelled=0 and (owner_email, customer) not in '\
            '(select owner_email, customer from customers_rate_owners) and customer=%s and start_date<curdate() and was_cancelled=0;'
    ...
    if request.method == 'POST':
        if 'submit_button' in request.form:
            ...
            if not date1 or not date2:
                ...
                return render_template('reserve_property.html', tb=tb,msg=msg,cur_date=cur_date)
            
            if (date1) < str(cur_date):
            ...
            if tb3:
                ...
                return render_template('reserve_property.html',msg=msg,cur_date=cur_date)
            ...
            if int(seats) > avai:
                ...
                return render_template('reserve_property.html',msg=msg,cur_date=cur_date)
    return render_template('reserve_property.html', tb=tb, msg=msg,cur_date=cur_date)
@app.route('/customer/customer_view_reserve', methods=['POST', 'GET'])
def customer_view_reserve():
 cursor = db_connection.cursor()
 cur_date= today
 customer=session["username"]
# query1 needs fix:
 query1="""select r.property_name, start_date, end_date, r.owner_email, was_cancelled as cancel, score 
from reserve r left join owners_rate_customers o on (r.owner_email, r.customer)
=(o.owner_email, o.customer) where r.customer= %s"""
 cursor.execute(query1,(customer,))
 tb = cursor.fetchall()
 print(tb)
 tb = [list(t) for t in tb]
 ...
 msg=''
 if not tb:
   msg='No reservation history'
   return render_template('customer_view_reserve.html', msg=msg)

 return render_template('customer_view_reserve.html', tb=tb, msg=msg)
